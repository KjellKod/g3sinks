...
TEST_F(RotateFileTest, setMaxLogSizeAndRotate_ValidNewName) {
  ...
  auto allFiles = LogRotateUtility::getLogFilesInDirectory(
      _directory, newFileName + ".log");
...
TEST_F(RotateFileTest, setMaxLogSizeAndRotate_EmptyNewName) {
  ...
  auto allFiles =
      LogRotateUtility::getLogFilesInDirectory(_directory, _filename + ".log");
  EXPECT_EQ(allFiles.size(), 1)
      << "direc: " << _directory << ", name: " << _filename;
}

namespace {
void RotateAndExpireOldLogs(std::string filename, std::string directory,
                            bool changeLogFile = false) {
...
TEST_F(RotateFileTest, rotateAndExpireOldLogs) {
  ...
  EXPECT_EQ(allFiles.size(), size_t{3})
      << " Failure " << ExtractContent(allFiles);
}

TEST_F(RotateFileTest, rotateAndExpireOldLogsWithoutTrailingSlashForDirectory) {
  auto filename =
      _filename.substr(0, _filename.size() - 1);  // remove the trailing '/'
  RotateAndExpireOldLogs(_filename, _directory);
  auto app_name = _filename + ".log";
